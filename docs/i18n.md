...
Le choix de la langue se trouve dans le menu du compte (votre identifiant, en haut à droite), avec le profil, le thème et la déconnexion.
...
Les chaînes source (`msgid`) sont écrites en français : le catalogue anglais contient les traductions ; dans le catalogue français, les `msgstr` restent vides et le texte source s'affiche tel quel.
...
Les prompts de l'IA (correction, générateur, entretiens, analyses) sont aussi traduits. Ce sont des dictionnaires par langue, dans `private.example/prompts/`. Pour les personnaliser sans toucher au dépôt, copiez ce dossier vers `private/prompts/` : les fichiers de `private/` sont prioritaires.
...
Le retour de l'IA est ainsi rédigé dans la langue de l'apprenant, avec des consignes et des critères traduits.
...
4. **Traduire les prompts IA** : ajoutez la clé `'es'` dans les fichiers `private.example/prompts/*.py` (et `private/prompts/` si vous l'utilisez)

5. **Ajouter la langue au menu du compte** (`nav-account` dans `app/templates/base.html`)

6. **Compiler et redémarrer** :
   ```bash
   pybabel compile -d translations
